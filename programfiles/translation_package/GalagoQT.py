...
class GalagoQT:
    ...
    def construct_galago_query_no_phrasing(self, weighted, keywords_with_score):
        if len(keywords_with_score)>1:
            ...
            for score, phrase in keywords_with_score:
                ...
                if len(wrapped_phrase)>=1:
                    ...
                    phrase_index+=1
            final_query = f"#combine {weights_str}({words_str})"
    ...
    def tranlate_phrase_combine(self, weighted, ph_txt, tran_top_k=2, trans_type='tt'):
        #kw_tokens = self.spacy_english_tokenizer_and_cleaner(ph_txt)
        ...
        if len(kw_tokens)>=1:
            ...
            for kw_token in kw_tokens:
                ...
                wrapped_trans = self.construct_galago_query_no_phrasing(weighted, q_trans_tokens)
                kw_tokens_translations.append((0.2, wrapped_trans))
            wrapped_kw = self.construct_galago_query_no_phrasing(weighted, kw_tokens_translations)
    
        return wrapped_kw
    
    def get_terms(self, ph_txt, tran_top_k=2):
        kw_tokens = ph_txt.split()
        words_str = ''
        for kw_token in kw_tokens:
            q_trans_tokens = self.get_tt_topk(kw_token, topk=tran_top_k)
            for score, phrase in q_trans_tokens:
                wrapped_phrase = phrase.strip()
                if len(wrapped_phrase)>=1:
                    words_str += ' ' + wrapped_phrase
